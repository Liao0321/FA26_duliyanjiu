...
// 1. 嘗試從全域變數讀取設定檔 (來自 index.html 載入的 firebase-config.js)
if (window.firebaseConfig) {
  firebaseConfig = window.firebaseConfig;
  if (window.TEACHER_EMAILS) {
    TEACHER_EMAILS = window.TEACHER_EMAILS;
  }
  hasConfig = true;
}

// 2. 若全域變數不存在，從 LocalStorage 載入 (本機設定頁面輸入快取)
if (!firebaseConfig) {
